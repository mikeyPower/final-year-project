...
with open(csvFile) as csvfile:
    ...
    with open("resolved_ip_to_dns"+now+".csv", "w") as myfile:
        ...
        writer.writerow(['Queried Ip','Hostname', 'alias-list', 'Ip'])
        with open("unresolved_ip_to_dns"+now+".csv", "w") as myfile1:
            writer1=csv.writer(myfile1)
            writer1.writerow(['Queried Ip, Error'])
            for i in ipAddress:
                try:
                    rdns=socket.gethostbyaddr(i)
                    dnsResolvedCount =dnsResolvedCount+1
                    writer.writerow([i,rdns[0], rdns[1], rdns[2]])
                    if rdns[0] in dictionary:
                        dictionary[rdns[0]].append(i)
                    else:
                        dictionary[rdns[0]] = [i]
                except Exception as e:
                    print >> sys.stderr, "dns exception " + str(e) + " for " + i
                    writer1.writerow([i, e])
...
print >>summary_fp, str(len(ipAddress)) + " Unique ips found"
