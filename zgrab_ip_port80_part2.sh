...
touch summary_zgrab_ip_p80__part2_$TIME_STAMP.txt
...
sed 1d $1 > ~/fyp/final-year-project/input4_part2.csv
while IFS=, read -a b;
do
...
done < ~/fyp/final-year-project/input4_part2.csv #removing header line

#Summary results
echo 'Ran zgrab_domain.sh at '$(date -d @$TIME_STAMP +"%d-%m-%Y %T") >> summary_zgrab_ip_p80__part2_$TIME_STAMP.txt
echo 'Input file: ' >> summary_zgrab_ip_p80__part2_$TIME_STAMP.txt
echo "$1" >> summary_zgrab_ip_p80__part2_$TIME_STAMP.txt
echo "Files created:" >> summary_zgrab_ip_p80__part2_$TIME_STAMP.txt
echo $2 >> summary_zgrab_ip_p80__part2_$TIME_STAMP.txt
