...
Signature Algorithm,Key Algorithm,Curve Id,Compression Method" > $2
...
sed 1d $1 > ~/fyp/final-year-project/input3.csv
while IFS=, read -a b;
do

    (cd ~/go/src/github.com/zmap/zgrab && echo "${b[0]}" | ./zgrab --port 443 --tls --http="/" --timeout 5 --output-file=banners3.json)

    ~/fyp/final-year-project/json_lookup.py ~/go/src/github.com/zmap/zgrab/banners3.json $2 443 ${b[1]}

done < ~/fyp/final-year-project/input3.csv #removing header line

#Summary results
echo 'Ran zgrab_domain.sh at '$(date -d @$TIME_STAMP +"%d-%m-%Y %T") >> summary_zgrab_ip_p443_$TIME_STAMP.txt
...
echo $2 >> summary_zgrab_ip_p443_$TIME_STAMP.txt
